...
import { Sparkles, Send, CheckCircle2, Gift } from "lucide-react";
...
export default function StickerAlbum() {
  ...
  const [stage, setStage] = useState<'catching' | 'letter' | 'form' | 'success' | 'bonus'>('catching');
  ...
  useEffect(() => {
    if (stage === 'success' && countdown > 0) {
      const timer = setTimeout(() => setCountdown(c => c - 1), 1000);
      return () => clearTimeout(timer);
    } else if (stage === 'success' && countdown === 0) {
      startBonusTransition();
    }
  }, [stage, countdown]);

  const startBonusTransition = () => {
  ...
  return (
    <>
      ...
      <div className="w-full max-w-3xl mx-auto flex flex-col items-center gap-8 relative z-10">
        ...
        {stage === 'success' && (
          <div className="w-full max-w-md animate-fade-in-up bg-white/90 backdrop-blur-sm p-10 rounded-3xl shadow-xl border-2 border-pastel-green text-center flex flex-col items-center gap-6">
            <CheckCircle2 size={64} className="text-pastel-green animate-bounce" />
            <div>
              <h3 className="text-2xl font-bold text-gray-800 mb-2">¡Mensaje guardado!</h3>
              <p className="text-gray-600 font-medium">Gracias por compartir este momento.</p>
            </div>
            <div className="mt-4 flex flex-col items-center gap-3">
              <Gift size={32} className="text-pastel-pink animate-pulse" />
              <p className="text-gray-500 font-medium animate-pulse">
                Redirigiendo a sorpresa final en {countdown}...
              </p>
            </div>
          </div>
        )}
